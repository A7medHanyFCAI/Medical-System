...
class Appointment(models.Model):
    ...
    def __str__(self):
        return f"{self.patient} - {self.start_date_time:%Y-%m-%d %H:%M} to {self.end_date_time:%H:%M}"
    ...
    def clean(self):
        ...
        if self.duration.total_seconds() <= 0:
            raise ValidationError("Appoints Duration Must Be Greater Than Zero")
        ...
        #need to check on the availbilites property 
        ...
        # to be refactored after the doctor model is created
        for available_time in availabilities:
            if available_time.weekday() != day_of_week:
                continue

            if start_time >= available_time.start_time and \
            end_time <= available_time.end_time:
                valid = True

        if not valid:
            raise ValidationError("Appointments is outside doctor's availability")
